...
│   ├── walls
│   ├── ceiling
│   └── grass_surface            → экспортируется как level_01_grass.gltf (автоматически!)
│       └── [Modifier] GeometryNodes: "GrassScatter" (БЕЗ Realize Instances!)
...
> **Как скрипт различает траву от обычной геометрии?** Автоматически — по наличию модификатора типа `NODES` (Geometry Nodes). Объекты с таким модификатором экспортируются отдельным файлом с `export_gpu_instances=True`. Никаких дополнительных настроек не нужно.
...
assets/models/levels/level_01_grass.gltf  ← трава (EXT_mesh_gpu_instancing, если есть GN-объекты)
scenes/levels/level_01.tscn               ← готовая сцена Godot с маркерами
```
...
Трава создаётся процедурно через модификатор **Geometry Nodes** — инстансы травинок распределяются по поверхности пола/рельефа.

> ⚠️ **Главная проблема с `export_apply=True`**: если применить модификатор через `Realize Instances`, каждая травинка становится **отдельной геометрией** в glTF. 1000 травинок × 6 tri = меш с 6000 уникальными треугольниками без шаринга данных — файл раздувается в десятки раз.

**Правильный подход**: экспортировать инстансы **без** `Realize Instances`, используя расширение glTF `EXT_mesh_gpu_instancing` (поддерживается в Blender 4.x). Godot 4 читает это расширение и создаёт `MultiMeshInstance3D` — меш хранится один раз, рендерится за один draw call.
...
> **Как это работает?** Скрипт [`export_level.py`](export_level.py) автоматически разделяет объекты коллекции `Geometry` на два типа: обычные меши → `level_01_geo.gltf` с `export_apply=True`; объекты с модификатором `NODES` (Geometry Nodes) → `level_01_grass.gltf` с `export_apply=False` + `export_gpu_instances=True`. Никаких отдельных коллекций не нужно.
...
#### 3. Нод-граф — БЕЗ Realize Instances
...
[Instance on Points]           ← Instance: объект grass_blade
      ...
      │
      ↓
[Group Output]                 ← НЕ добавляй Realize Instances!
```

> **Почему без `Realize Instances`?** Без него Blender экспортирует инстансы через `EXT_mesh_gpu_instancing` — меш травинки хранится **один раз**, а в glTF записываются только матрицы трансформаций каждого инстанса. Файл остаётся компактным.
...
Вынеси параметры наружу, чтобы менять их без входа в граф:
...
### Экспорт: сохранение инстансов

**Вариант A — ручной экспорт (Blender 4.x):**
```
File → Export → glTF 2.0
  Apply Modifiers: OFF   ← НЕ применяем модификаторы
  Geometry → Instancing: ON   ← экспортирует EXT_mesh_gpu_instancing
```

**Вариант B — через скрипт (рекомендуется, всё автоматически):**

Скрипт [`export_level.py`](export_level.py) сам определяет объекты с Geometry Nodes по наличию модификатора `NODES` и вызывает для них отдельный экспорт:

```python
# Автоматически в export_level.py:
# Обычные меши:
bpy.ops.export_scene.gltf(filepath=..., export_apply=True, ...)
# Объекты с Geometry Nodes (трава):
bpy.ops.export_scene.gltf(filepath=..., export_apply=False, export_gpu_instances=True, ...)
```

> **Результат в glTF**: меш `grass_blade` записан **один раз** (~200 байт геометрии), а 1000 инстансов — это массив из 1000 матриц 4×4 (~64 КБ вместо ~1.2 МБ дублированной геометрии).

### Импорт в Godot

Godot 4.x читает `EXT_mesh_gpu_instancing` и создаёт `MultiMeshInstance3D` автоматически:

```
level_01.gltf импортируется →
  MeshInstance3D: floor_main
  MultiMeshInstance3D: grass_surface   ← трава как GPU instancing
```

`MultiMeshInstance3D` рендерит тысячи инстансов за **один draw call** — оптимально для травы.

### Оптимизация травы

| Приём | Описание |
|-------|---------|
| **Без Realize Instances** | Главное правило — меш хранится один раз, инстансы дёшевы |
| **Density Map** | Подключи текстуру ч/б к Density — трава только там, где нужно |
| **Polycount травинки** | 4–8 tri — GPU instancing рендерит их эффективно |
| **Vertex Colors** | Используй `Set Material` + Vertex Color для вариативности цвета без доп. текстур |
...
Размер в glTF:   меш × 1 + матрицы × 1200  ≈  несколько КБ  ✓
...
- [ ] Нод `Realize Instances` **отсутствует** в графе
...
- [ ] В Godot проверить, что импортировался `MultiMeshInstance3D` (не `MeshInstance3D`)
